...
module.exports.handleEvent = async function ({
  ...
  try {
    ...
    if (matchedPrefix) {
      ...
      const userQuery =
        raw.slice(matchedPrefix.length).trim();

      if (!userQuery) return;


      const res = await axios.get(
        `${simsim}/simsimi?text=${encodeURIComponent(userQuery)}&senderName=${encodeURIComponent(senderName)}`
      );
